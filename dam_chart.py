...
def query(sql):
    ...
    rows = []
    # ВАЖЛИВО: стрипаємо тільки \n, не весь whitespace — бо провідні/кінцеві
    # таби є значущими роздільниками порожніх (NULL) полів і .strip()
    # помилково їх також видаляє, ламаючи парсинг.
    for line in r.stdout.strip("\n").split("\n"):
        if line == "":
            continue
        parts = line.split("\t")
        # Рядок-агрегат без GROUP BY завжди повертається навіть для 0 рядків;
        # якщо всі поля порожні/NULL — даних немає, відкидаємо рядок.
